...
function productRouter(adminController){
  ...
  router.post("/", upload.single("uploadedImage"), resize, adminController.addProduct);
  router.post("/codeCheck", adminController.checkProductCode);
  router.get("/:id", adminController.getProduct);
  router.patch("/:id", upload.single("uploadedImage"), resize, adminController.updateProduct);
